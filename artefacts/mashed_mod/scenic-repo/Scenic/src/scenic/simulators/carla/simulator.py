...
# logging.basicConfig(
#     filename="/home/paperspace/Desktop/carla_scratchpad/logging/sensor_attributes.log",
#     filemode="w",
#     level=logging.INFO,
...
class CarlaSimulation(DrivingSimulation):
    ...
    def setup(self):
        ...
        if self.render:
            ...
            self.displayDim = (self.screen_width, self.screen_height)
    ...
    def createObjectInSimulator(self, obj):
        ...
        if obj.sensors:
            for sensor_key, sensor in obj.sensors.items():
                ...
                # print(sensor_bp.id, sensor_bp.tags)

                carla_sensor = self.world.spawn_actor(
                    sensor_bp, sensor.transform, attach_to=obj.carlaActor
                )
                # uncomment me for server-client interaction
                # if sensor_bp.id == "sensor.camera.rgb":
                #     carla_sensor.listen(sensor.on_data_rgb)
                #     # logging.info(dir(self))
                #     self.pedestrian_detected = sensor.pedestrian_detected
                #     self.world.tick()
                #     print(f"in sim/carla/sim.py {self.pedestrian_detected}")
                # else:
