...
# model_path = os.path.join(current_dir, "model", "yolov5s.pt")
# yolo_model = torch.hub.load("ultralytics/yolov5", "custom", path=model_path, force_reload=True)  # PyTorch
...
# setup_logging('timing_log_kid_dist_15')
...
def checkPedestrianDectectedFlag(observations, pedestrian_position, ego_position):
    ...
    if not person_detections.empty:
        print('check pedestrian detected flag')
        return True
    return False

def adjust_based_on_confidence(confidence):
    min_val, max_val = 0.30, 0.85
    if confidence >= max_val:
        return 1
    else:
        scaled_confidence = ((confidence - min_val) / (max_val - min_val)) * 0.75
        return scaled_confidence

def checkCautionBehaviour(observations, pedestrian_position, ego_position):
    resized_frame = cv2.resize(observations, (640, 640))
    results = yolo_model(resized_frame)
    df = results.pandas().xyxy[0]
    ...
    if person_low_con_det.empty:
        ...
        print(f'in check caution: {confidence_value}')
        return adjust_based_on_confidence(confidence_value)


# monitor RecordingMonitor(ego, path=localPath("/home/paperspace/Desktop/save_dir/sensors_branch"), recording_start=5, subsample=5)
